...
// Migrated from the original desktop exercise. Each now_us() call advances
// simulated time by one microsecond; this is not a physical clock.
...
class MockClock : public Clock {
    public:
        ...
        void delay_us(std::uint32_t duration) const override {
            ...
            while (!dht11::elapsed_at_least(now, start, duration)) {
                now = now_us();
